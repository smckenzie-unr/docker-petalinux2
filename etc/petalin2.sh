...
docker run -ti "${SET_X_SERVER[@]}" "${SET_MIRROR_PATH[@]}" -v "$PWD":/home/petalinux/project:z -v "$PWD/tftpboot":/tftpboot:z --rm -u petalinux "${OVERRIDE_ENTRYPOINT[@]}" docker_petalinux2:"${latest}" "${SET_DOCKER_COMMAND[@]}"
